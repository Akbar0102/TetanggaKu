...
public class LokasiEntertainment extends AppCompatActivity implements OnMapReadyCallback, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {
    ...
    private List<Marker> marker_enter = new ArrayList<Marker>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lokasi_entertainment);
    ...
    */
    ...
    public void onLocationChanged(Location location) {
        ...
        //cek_radius(location, mCircle, marker_enter);
    }
        ...
        if (mGoogleMap == null)
            mMapView.getMapAsync(new OnMapReadyCallback() {
                ...
                public void onMapReady(GoogleMap googleMap) {
                    ...
                    StringRequest stringRequest = new StringRequest(Request.Method.GET, URLs.URL_GETPOIENTER,
                            new Response.Listener<String>() {
                                @Override
                                public void onResponse(String response) {
                                    try {
                                        JSONObject obj = new JSONObject(response);
                                        JSONArray wenter = obj.getJSONArray("entertainment");
                                        for (int i = 0; i < wenter.length(); i++) {
                                            //getting the json object of the particular index inside the array
                                            JSONObject wilObject = wenter.getJSONObject(i);

                                            //marker rumah warga
                                            enter = new LatLng(wilObject.getDouble("latitude"), wilObject.getDouble("longitude"));
                                            Marker marker = mGoogleMap.addMarker(new MarkerOptions().position(enter)
                                                    .title(wilObject.getString("nama_tempat"))
                                                    .snippet(wilObject.getString("alamat"))
                                                    .icon(bitmapDescriptorFromVector(getApplicationContext(), R.drawable.ic_enter)));
                                            marker_enter.add(marker);
                                        }
                                        ...
                                        mGoogleMap.animateCamera(CameraUpdateFactory.newLatLngZoom(enter, 17));
