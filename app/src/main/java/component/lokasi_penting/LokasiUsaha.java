...
public class LokasiUsaha extends AppCompatActivity implements OnMapReadyCallback, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {
    ...
    private List<Marker> marker_usaha = new ArrayList<Marker>();
    private Circle mCircle;
    private LatLng latLng;
    private LatLng usaha;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_lokasi_usaha);
    ...
    */
    ...
    public void onLocationChanged(Location location) {
        ...
        //cek_radius(location, mCircle, marker_usaha);
    }
        ...
        if (mGoogleMap == null)
            mMapView.getMapAsync(new OnMapReadyCallback() {
                ...
                public void onMapReady(GoogleMap googleMap) {
                    ...
                    StringRequest stringRequest = new StringRequest(Request.Method.GET, URLs.URL_GETPOIUSAHA,
                            new Response.Listener<String>() {
                                @Override
                                public void onResponse(String response) {
                                    try {
                                        JSONObject obj = new JSONObject(response);
                                        JSONArray wusaha = obj.getJSONArray("usaha");
                                        for (int i = 0; i < wusaha.length(); i++) {
                                            //getting the json object of the particular index inside the array
                                            JSONObject wilObject = wusaha.getJSONObject(i);

                                            //marker rumah warga
                                            usaha = new LatLng(wilObject.getDouble("latitude"), wilObject.getDouble("longitude"));
                                            Marker marker = mGoogleMap.addMarker(new MarkerOptions().position(usaha)
                                                    .title(wilObject.getString("nama_tempat"))
                                                    .snippet(wilObject.getString("alamat"))
                                                    .icon(bitmapDescriptorFromVector(getApplicationContext(), R.drawable.ic_usaha)));
                                            marker_usaha.add(marker);
                                        }
                                        ...
                                        mGoogleMap.animateCamera(CameraUpdateFactory.newLatLngZoom(usaha, 17));
